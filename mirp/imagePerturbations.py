...
def randomise_roi_contours(
    ...
    for roi_ind in np.arange(0, len(roi_list)):
        ...
        #print("res_img_obj",res_img_obj)
        #if np.isnan(res_img_obj.voxel_grid).any():
        #    print("np.isnan(res_img_obj)",np.isnan(res_img_obj.voxel_grid).any())
        #print("res_roi_obj",res_roi_obj)
        #if np.isnan(res_roi_obj[0].roi.voxel_grid).any():
        #    print("np.isnan(res_roi_obj)",np.isnan(res_roi_obj[0].roi.voxel_grid).any())
